...
export interface NeonMeshProps {
    ...
    description?: string;
    className?: string;
    /** Force theme instead of relying on prefers-color-scheme. */
    mode?: "light" | "dark";
    /** Hide the built-in title/subtitle/description overlay entirely. */
    showOverlay?: boolean;
}
...
export function NeonMesh({
    ...
    useEffect(() => {
        if (mode) return; // mode prop overrides system pref
    ...
    const isDarkMode = mode ? mode === "dark" : systemDark;
    ...
    useEffect(() => {
        ...
        // Listen on window so the mesh reacts even when the container is
        // pointer-events-none (background usage).
    ...
    return (
        ...
        >
            ...
            {showOverlay && (title || subtitle || description) && (
